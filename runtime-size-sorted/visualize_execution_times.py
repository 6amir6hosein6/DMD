...
def visualize_execution_times():
    ...
    configs = [
        ('runtime-size-sorted/execution_times_b256_w8.csv', 256, 8, False),
        ('runtime-size-sorted/execution_times_b256_w8_sorted.csv', 256, 8, True),
    ]
    ...
    for col in columns:
        ...
        for i, (df, (_, batch, worker, is_sorted)) in enumerate(zip(dfs, configs)):
            ...
            plt.scatter(x_values, y_values, label=f'b{batch}_w{worker}_{is_sorted}', alpha=0.7, color=colors[i])
        ...
        plt.savefig(f'runtime-size-sorted/{safe_col_name}_plot.png', bbox_inches='tight')
        plt.close()
